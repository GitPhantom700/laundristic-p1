...
- **Note for Claude Code (Admin App):** The user has requested to build an Admin App in the future. They specifically want to use the "Dusty Blue & Sand" UI design mockup for it. Please retain this instruction somewhere safe so we can start working on it using that design once the current application is finished.
