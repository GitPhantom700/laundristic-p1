...
- **P4.5 [CC]** Auto-delete garments on batch close: `received` garments deleted from catalog on check-in completion; `missing` garments deleted when batch fully resolves to `closed`. Domain changes in `closeCheckIn` + missing-item resolution in `domain.ts`/`storage.ts`. ✓ Catalog only shows garments in user's possession.
