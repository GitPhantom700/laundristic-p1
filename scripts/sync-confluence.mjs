...
const GITHUB_RAW_BASE = 'https://raw.githubusercontent.com/GitPhantom700/laundristic/main/';
...
if (!SPACE_KEY || !DOMAIN || !EMAIL || !TOKEN) {
  console.error('Error: Missing required environment variables. Please set CONFLUENCE_SPACE_KEY, CONFLUENCE_DOMAIN, CONFLUENCE_EMAIL, and CONFLUENCE_API_TOKEN in your .env file.');
  process.exit(1);
}
...
function processMarkdown(filePath, markdown) {
  ...
  let processedMd = markdown.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, (match, alt, imagePath) => {
    if (imagePath.startsWith('http://') || imagePath.startsWith('https://')) {
      return match;
    }
    let resolvedPath = path.posix.join(dirPath.replace(/\\/g, '/'), imagePath);
    const absoluteUrl = `${GITHUB_RAW_BASE}${resolvedPath}`;
    return `![${alt}](${absoluteUrl})`;
  });

  // 3. Transform GitHub Admonitions to Confluence Macros
  processedMd = processedMd.replace(/^> \[\!(NOTE|TIP|IMPORTANT|WARNING|CAUTION)\]\n((?:> .*\n?)*)/gm, (match, type, body) => {
    let acType = 'info';
    if (['WARNING', 'CAUTION', 'IMPORTANT'].includes(type)) acType = 'warning';
...
async function syncFile(filePath) {
  ...
  if (DRY_RUN) {
    console.log(`[DRY RUN] Would sync "${title}" to space ${SPACE_KEY}. HTML length: ${htmlContent.length} chars.`);
    return;
  }
  ...
  if (searchResult.results && searchResult.results.length > 0) {
    ...
    console.log(`Page exists (ID: ${page.id}, Version: ${currentVersion}). Updating...`);
    ...
    console.log(`Success: Updated page "${title}" to version ${currentVersion + 1}.`);
  } else {
...
async function run() {
  console.log(`Starting Confluence Sync (Space: ${SPACE_KEY})${DRY_RUN ? ' [DRY RUN]' : ''}`);
